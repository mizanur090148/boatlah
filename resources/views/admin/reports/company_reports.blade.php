            ...
            <div class="col-md-12">
                ...
                <div class="panel">
                    <div class="panel-body">
                        {{Form::open(array('url'=>'/admin/reports/company_reports_post', 'files'=>true,'id'=>'exampleSummaryForm','autocomplete'=>'off','class'=>'form-horizontal'))}}
                            ...
                            <div class="col-sm-7">
                                <select class="form-control" name="name">
                                    ...
                                    @foreach($companies as $company)
                                        <option value="{{$company->user_id}}">{{$company->user->name}} ({{$company->user->email}})</option>
                                    @endforeach
                                </select>
                                <span class="validator_output <?php if($errors->first('name')!=null) echo "help-block"?>">{{ $errors->first('name') }}</span>
                            </div>
                            <div class="col-sm-2">
                                <button type="submit" class="btn btn-primary" id="validateButton3">Go To Company Report</button>
                            </div>
                        </div>
                        {{Form::close()}}
                    </div>
