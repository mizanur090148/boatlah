            ...
            <div class="col-md-12">
                ...
                <div class="panel">
                    <div class="panel-body">
                        {{Form::open(array('url'=>'/admin/reports/user_reports_post', 'files'=>true,'id'=>'exampleSummaryForm','autocomplete'=>'off','class'=>'form-horizontal'))}}
                            ...
                            <div class="col-sm-7">
                                <select class="form-control" name="name">
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{$user->user_id}}">{{$user->user->name}} ({{$user->user->email}})</option>
                                    @endforeach
                                </select>
                                <span class="validator_output <?php if($errors->first('name')!=null) echo "help-block"?>">{{ $errors->first('name') }}</span>
                            </div>
                            <div class="col-sm-2">
                                <button type="submit" class="btn btn-primary" id="validateButton3">Go To User Report</button>
                            </div>
                        </div>
                        {{Form::close()}}
                    </div>
